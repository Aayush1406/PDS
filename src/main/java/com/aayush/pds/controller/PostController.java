...
public class PostController {
  ...
  public ResponseEntity<Map<String, Integer>> createPost(
    @RequestHeader int userId,
    @RequestBody PostEncrypted postEncrypted
  ) throws Exception {
    setSharedKeyOfUser(userId); // Setting the shared key of client everytime
    String decryptedPostContent = aes.decryptData(
      dhKeys.getDhSHAServerSharedKey(),
      postEncrypted.getEncryptedContent()
    ...
    String rsaPostContentDecryptedSignature = rsaKeys.getRSADecryptedData(
      postEncrypted.getPostDigitalSignature()
    ); // decrypt the digital signature to get SHA256
  ...
  public ResponseEntity<Map<String, String>> getPostList(
    ...
    String viewerRSAPublicKey = postService.getViewerRSAPublicKey(
      post.getUserId()
    ); //fetch viewer rsa public from usertable
    ...
    for (int i = 0; i < postIdsList.size(); i++) {
      ...
      String decryptedPostContent = aes.decryptData(
        contentSharedKey,
        encryptedContent
      ); //decrypting the post with shared key of user who created the actual post.
    ...
    String encryptedPostContentList = aes.encryptData(
      viewerSharedKey,
      jsonStrArrayOfPost
    ); //encrypting the json string using viewer shared secret key
    ...
    String postContentDigitalSignature = rsaKeys.encrypt(
      postContentListSHA256,
      viewerRSAPublicKey
    ); // encrypting SHA256 with viewer RSA public key.

    System.out.println("Encrypted Post List = " + encryptedPostContentList);
    System.out.println(
      "Post Content Digital Signature = " + postContentDigitalSignature
    );
  ...
  public String convertListToJson(List<PostDTO> list)
