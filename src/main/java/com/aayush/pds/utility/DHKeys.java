...
public class DHKeys {
  ...
  public void setClientPrimeNumber(BigInteger clientPrimeNumber)
    throws IOException {
  ...
  public void setServerSharedKey(BigInteger serverSharedKey)
    throws IOException {
  ...
  public void generateDHExchangeKeys() throws IOException {
    ...
    for (String fileNamePointer : fileNames) {
      ...
      if (f.exists() == false) {
        ...
        dhServerPublicKey =
          getClientG().modPow((dhServerPrivateKey), getClientPrimeNumber());
  ...
  public String generateSharedKey(UserEntity userEntity)
    throws IOException, NoSuchAlgorithmException {
    dhServerSharedKey =
      new BigInteger(userEntity.getUserPublicKey())
        .modPow((getServerPrivateKey()), getClientPrimeNumber());
    setServerSharedKey(dhServerSharedKey);
    System.out.println(
      "Shared Key Final in Hex format = " + getServerSharedKey().toString(16)
    );
    String SHAServerSharedKey = generateSHA256(
      getServerSharedKey().toString(16)
    );
  ...
  public String generateSHA256(String secretKey)
  ...
  public void createPemFile(String pemData, String fileName)
    throws IOException {
  ...
  public void readPemFile(File file) throws IOException {
    ...
    try (
      ...
      while ((line = reader.readLine()) != null) {
        if (
          line.equals("-----BEGIN-----") == false &&
          line.equals("-----END-----") == false
        ) {
          value = line;
          flag = true;
        }
